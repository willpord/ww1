...
counter = 2  # dont use this one, just incase we lose the other ones
map = """
# = france    |                         %%%%%%%%%%%%%%%%%   %%%%%  ################################################
& = uk        |                          %%%%%%%%%%%%%%%      ####################################################    
^ = serbia    |                           %%%%%%%%%%%%%% %    #####################################################
...
# fmt: on
def map_string():  # __str__() next time buddy you cant do this
    mapbutstring = ""  # tho idk what a class would do why would i do that
    for i in map:
        for j in i:
            mapbutstring = mapbutstring + j
    return


# DO ALL THIS WITH JSON alr
class province:  #
    def __init__(
        self,
        location,
...
class economy:
    ...
    def update(self, parent):
        ...
        self.costs = parent.arms.soldiers * parent.arms.salary + parent.tech.budget
        ...
        Military:
        Personnel Costs: {parent.arms.soldiers * parent.arms.salary}
        ...
        while True:
            ...
            match action[0]:
                case "attack":
                    self.attack(subject)
                case "fortify":
                    # come back when map is figured out
                    pass
                case "research":
                    self.tech.budget += action[1]
                case "recruit":
